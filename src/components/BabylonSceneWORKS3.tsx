...
// const BabylonScene = ({ rotationTrigger: _rotationTrigger, direction: _direction, permissionsGranted = true, onSceneLoaded, initialOrientation = null, onRequestCameraAlignment }: BabylonSceneProps) => {
...
//     dome.onReady = () => {
//       // PANORAMA STARTING POINT: Adjust the value 0.75 to rotate the starting view
//       // Larger values rotate right, smaller values rotate left
//       // Range: 0 to 2π (0 to ~6.28)
//       pivotIOS.rotate(axis, Math.PI - 4.73, 1);

//       // Enable orientation handling now that panorama is ready
//       panoramaReady = true;
...
//     // Use initialOrientation from camera alignment as baseline
//     // If user skipped alignment, capture baseline on first frame
//     let baselineQuaternion: Quaternion | null = initialOrientation ? initialOrientation.clone() : null;
//     let needToSetBaseline = initialOrientation === null;
//     let isFirstFrame = true; // Track first frame to set camera to identity
//     let panoramaReady = false; // Track when panorama is fully loaded

//     // Handle device orientation with averaging
//     const handleOrientation = (event: DeviceOrientationEvent) => {
//       if (!useGyroRef.current || !cameraRef.current || !panoramaReady) return;
...
//       // If no initialOrientation was provided, capture baseline on first frame
//       if (needToSetBaseline && baselineQuaternion === null) {
//         baselineQuaternion = finalQuaternion.clone();
//         needToSetBaseline = false;
//       }

//       // On first frame, always set camera to identity
//       // This ensures panorama starts at the same position for everyone
//       if (isFirstFrame) {
//         isFirstFrame = false;
...
//       // If we have a baseline (either from initialOrientation or first frame)
...
//         const deltaRotation = baselineInverse.multiply(finalQuaternion);

//         // Extract Euler angles from delta rotation
//         const euler = deltaRotation.toEulerAngles();

//         // Create a new rotation using only yaw and pitch, but keep roll at 0
//         // This keeps the horizon level while allowing left/right (yaw) and up/down (pitch) movement
//         const leveledRotation = Quaternion.RotationYawPitchRoll(euler.y, euler.x, 0);

//         // Apply the leveled rotation to camera
//         camera.rotationQuaternion = leveledRotation;
//       }
//     };
...
//     if (isManualMode) {
//       // Re-enable gyroscope and request camera alignment
//       setIsManualMode(false);
//       useGyroRef.current = true;
//       setShowModal(false);
//       // Request camera alignment to recapture baseline
//       if (onRequestCameraAlignment) {
//         onRequestCameraAlignment();
//       }
//     } else {
