...
//     camera.rotationQuaternion = initialOrientation ? initialOrientation.clone() : new Quaternion();
...
//     // Store the first device orientation quaternion to use as baseline for relative rotation
//     let baselineQuaternion: Quaternion | null = null;
//     let rotationOffset: Quaternion | null = null; // Pre-calculated offset to avoid recalculating every frame
...
//       // On first reading, capture baseline and calculate offset ONCE
//       if (!firstOrientationReceived) {
//         baselineQuaternion = finalQuaternion.clone();
//         firstOrientationReceived = true;

//         // Calculate rotation offset once (from current device orientation to desired initial orientation)
//         if (initialOrientation) {
//           // rotationOffset = initialOrientation * inverse(baselineQuaternion)
//           // Using clone to avoid mutating the original
//           const baselineInverse = baselineQuaternion.clone();
//           baselineInverse.invertInPlace();
//           rotationOffset = initialOrientation.multiply(baselineInverse);
//           camera.rotationQuaternion = initialOrientation.clone();

//           // Debug logging
//           console.log('BabylonScene - Initial Orientation:', {
//             x: initialOrientation.x.toFixed(3),
//             y: initialOrientation.y.toFixed(3),
//             z: initialOrientation.z.toFixed(3),
//             w: initialOrientation.w.toFixed(3)
//           });
//           console.log('BabylonScene - Baseline (device orientation at start):', {
//             x: baselineQuaternion.x.toFixed(3),
//             y: baselineQuaternion.y.toFixed(3),
//             z: baselineQuaternion.z.toFixed(3),
//             w: baselineQuaternion.w.toFixed(3)
//           });
//           console.log('BabylonScene - Calculated Offset:', {
//             x: rotationOffset.x.toFixed(3),
//             y: rotationOffset.y.toFixed(3),
//             z: rotationOffset.z.toFixed(3),
//             w: rotationOffset.w.toFixed(3)
//           });
//         } else {
//           rotationOffset = new Quaternion(); // Identity quaternion
//           camera.rotationQuaternion = finalQuaternion.clone();
//         }
//       } else if (rotationOffset) {
//         // Apply pre-calculated rotation offset (no recalculation = no flicker)
//         camera.rotationQuaternion = rotationOffset.multiply(finalQuaternion);
//       } else {
//         // Fallback
//         camera.rotationQuaternion = finalQuaternion;
//       }
//     };

//     // Add event listeners - permissions already handled by parent component
//     // IMPORTANT: Only use 'deviceorientation' (NOT 'deviceorientationabsolute')
//     // deviceorientationabsolute uses compass which we want to avoid
//     // deviceorientation gives relative gyroscope data which is what we want
